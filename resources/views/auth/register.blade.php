                ...
                <div class="w-100" style="max-width: 360px; margin: auto;">
                    ...
                    <form action="{{ route('register.submit') }}" method="POST" onsubmit="return validatePasswordMatch();">
                        ...
                        <input type="text" name="users_name"
                            class="form-control mb-3 @error('users_name') is-invalid @enderror" placeholder="Full Name"
                            required value="{{ old('users_name') }}">
                        @error('users_name')
                            <div class="text-danger small">{{ $message }}</div>
                        @enderror

                        <input type="email" name="users_email"
                            class="form-control mb-3 @error('users_email') is-invalid @enderror" placeholder="Email"
                            required value="{{ old('users_email') }}">
                        @error('users_email')
                            <div class="text-danger small">{{ $message }}</div>
                        @enderror

                        <input type="text" name="users_phone"
                            class="form-control mb-3 @error('users_phone') is-invalid @enderror" placeholder="Phone Number"
                            pattern="[0-9]+" maxlength="15" required value="{{ old('users_phone') }}">
                        @error('users_phone')
                            <div class="text-danger small">{{ $message }}</div>
                        @enderror

                        <input type="text" name="users_address"
                            class="form-control mb-3 @error('users_address') is-invalid @enderror" placeholder="Address"
                            required value="{{ old('users_address') }}">
                        @error('users_address')
                            <div class="text-danger small">{{ $message }}</div>
                        @enderror

                        <div class="position-relative mb-3">
                            <input type="password" id="password" name="users_password"
                        ...
                        <div class="position-relative mb-3">
                            <input type="password" id="password_confirmation" name="users_password_confirmation"
                                class="form-control @error('users_password_confirmation') is-invalid @enderror"
                                placeholder="Confirm Password" required>
                            <span class="toggle-password" onclick="togglePassword('password_confirmation', this)"><i
                                    class="fa fa-eye"></i></span>
